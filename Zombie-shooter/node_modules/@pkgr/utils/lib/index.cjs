...
function executeNodeScript(scriptPath, url) {
  ...
  child.on("close", (code) => {
    if (code !== 0) {
      console.log();
      console.log(picocolors__default["default"].red("The script specified as BROWSER environment variable failed."));
  ...
  if (shouldTryOpenChromiumWithAppleScript) {
    ...
    for (const chromiumBrowser of supportedChromiumBrowsers) {
      try {
        ...
        node_child_process.execSync('osascript ../openChrome.applescript "' + encodeURI(url) + '" "' + chromiumBrowser + '"', {
          cwd: _dirname,
          stdio: "ignore"
        });
        return true;
...
const tryGlob = (paths, options = {}) => {
  ...
  return paths.reduce((acc, pkg) => [
    ...acc,
    ...isGlob__default["default"](pkg) ? tryRequirePkg("tiny-glob/sync")(pkg, {
      absolute,
      cwd: baseDir
...
const findUp = (searchEntry, searchFile = "package.json") => {
  ...
  searchEntry = path__default["default"].resolve(fs__default["default"].statSync(searchEntry).isDirectory() ? searchEntry : path__default["default"].resolve(searchEntry, ".."));
  do {
