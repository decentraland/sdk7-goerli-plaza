...
export async function runChecks(
	...
	const validOrigin = (TESTS_ENABLED && (metadata.realm.hostname === 'localhost' || metadata.realm.hostname === 'LocalPreview'))
		? true
		: checkOrigin(req)
	if (!validOrigin) {
	...
	const validCatalystPos: boolean | undefined = (TESTS_ENABLED && (metadata.realm.hostname === 'localhost' || metadata.realm.hostname === 'LocalPreview'))
		? true
		: await checkPlayer(
			userAddress,
