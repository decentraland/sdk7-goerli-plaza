...
async function emitFile(fileName, services, cfg) {
    ...
    function loadJsLib(lib) {
        ...
        if (path) {
            ...
            loadedLibs.push({
                ...
                fullPath: (0, path_1.resolve)(ts.sys.getCurrentDirectory(), path),
                content,
    ...
    function getOutFile(path) {
        let f = out.get(path);
        if (!f) {
            f = { path, fullPath: (0, path_1.resolve)(ts.sys.getCurrentDirectory(), path) };
            out.set(path, f);
        }
        return f;
    }
    ...
    for (const [, file] of out) {
        if (file.path.endsWith('.js') &&
            ...
            loadedLibs.push({
                ...
                fullPath: (0, path_1.resolve)(ts.sys.getCurrentDirectory(), fileName),
...
function getConfiguration(packageJson, sceneJson) {
    ...
    if (!isDecentralandLib && process.env.NO_DEFAULT_LIBS !== '1') {
        ...
        const decentralandEntryPoint = process.env.SDK_PATH ||
