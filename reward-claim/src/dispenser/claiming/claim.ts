...
import { activeUiType, alreadyClaimedUi, captchaUi, endedUi, errorUi, genericFinishedUi, notStartedUi, outOfStockUi, simpleErrorUi, successUi, UiType, waitingUi } from './ui/dispenserUi'
...
export async function claimToken(campaign: ClaimConfigInstType, campaign_key: string, captcha_id?: string, captcha_value?: string) {
    ...
    let result = await requestToken(campaign, campaign_key, captcha_id, captcha_value)
    return result
}

async function requestToken(campaign: ClaimConfigInstType, campaign_key: string, captcha_id?: string, captcha_value?: string) {
    let METHOD_NAME = 'requestToken.'

    const url =
        (CONFIG.CONFIG_CLAIM_TESTING_ENABLED) ?
...
async function processResponse(response: any, campaign_key: string, campaign: ClaimConfigInstType) {
    ...
    if (
        (response.body as string).includes &&
        (response.body as string).toLowerCase().includes('captcha')
    ) {

        console.log("checking captcha")
        getCaptcha(campaign, campaign_key)
        // return result
        return false
    }
    try {
        ...
        if (json.ok === false) {
            ...
            // simpleErrorUi(json.code ? json.code : 'Invalid response')
            if (
                (
            ...
            simpleErrorUi(json.error ? json.error : json.code ? json.code : 'Invalid response')
            return false
        }
        ...
        // result success
            ...
            if (json.data[0].assigned_at) {
                alreadyClaimed.push(campaign_key)
                if (new Date(json.data[0].assigned_at).getTime() < (new Date().getTime() - 1000 * 60 * 2)) {
                    alreadyClaimedUi()
                    return true;
                }
            }
        ...
        simpleErrorUi(JSON.stringify(response.body))
        return false
    }
...
async function getCaptcha(campaign: ClaimConfigInstType, campaign_key: string) {

    const captchaResponse = await fetch(ClaimConfig.rewardsServer + '/api/captcha', { method: "POST" })
    const captchaBody = await captchaResponse.json()
    
    captchaUi({...captchaBody.data, campaign, campaign_key})
