...
export async function validateCaptcha(captcha: string, captcha_id: string, campaign: ClaimConfigInstType, campaign_key: string) {
	const user = await getUserData({})
	let realm = await getRealm({})
	console.log('realm is', realm.realmInfo)
	if (!user || !user.data || !user.data.publicKey || !realm) return
