...
app.get('/get-scores', async (req: Request & DecentralandSignatureData<Metadata>, res: Response) => {
  try {
    req.baseUrl = SERVER_BASE_URL
    return signedFetch({ expiration: VALID_SIGNATURE_TOLERANCE_INTERVAL_MS })(req, res, async () => {
      try {
...
app.post('/publish-score', async (req: Request & DecentralandSignatureData<Metadata>, res: Response) => {
  try {
    req.baseUrl = SERVER_BASE_URL
    return signedFetch({ expiration: VALID_SIGNATURE_TOLERANCE_INTERVAL_MS })(req, res, async () => {
      try {
