...
export async function runChecks(
  ...
  const validOrigin = (TESTS_ENABLED && metadata.realm.hostname === 'localhost')
  ? true
  : checkOrigin(req)
  if (!validOrigin) {
  ...
  const validCatalystPos: boolean | undefined = (TESTS_ENABLED && metadata.realm.hostname === 'localhost')
    ? true
    : await checkPlayer(
      userAddress,
