import { Animator, AvatarAnchorPointType, AvatarAttach, CameraMode, CameraModeArea, CameraType, ColliderLayer, Entity, GltfContainer, InputAction, PointerEvents, Transform, VisibilityComponent, engine, pointerEventsSystem } from "@dcl/sdk/ecs"
...
export function createBall(position: Vector3, cannonMaterial: CANNON.Material, cannonWorld: CANNON.World) {
    ...
    body = new CANNON.Body({
        mass: 1, // kg
        position: new CANNON.Vec3(Transform.get(ball).position.x, Transform.get(ball).position.y, Transform.get(ball).position.z), // m
...
function playerPickup(ball: Entity, body: CANNON.Body): void {
    ...
    body.position.set(Transform.get(engine.CameraEntity).position.x, Transform.get(engine.CameraEntity).position.y, Transform.get(engine.CameraEntity).position.z + 1)
    Transform.getMutable(ball).position = Vector3.create(Transform.get(engine.CameraEntity).position.x, Transform.get(engine.CameraEntity).position.y, Transform.get(engine.CameraEntity).position.z + 1)
...
export function setGlow(ball: Entity, isOn: boolean): void {
    const ballComponent = Ball.getMutable(ball)
    isOn ? Transform.getMutable(ballComponent.glowEntity).scale = Vector3.create(1, 1, 1) : Transform.getMutable(ballComponent.glowEntity).scale = Vector3.create(0, 0, 0)
}
