...
import { activeUiType, alreadyClaimedUi, endedUi, errorUi, notStartedUi, successUi, ClaimingUiType, waitingUi } from './ui/dispenserUi'
...
export async function claimToken(campaign: ClaimConfigInstType, campaign_key: string) {
    ...
    if(!CONFIG_CLAIM_TESTING_ENABLED){
...
async function requestToken(campaign: ClaimConfigInstType, campaign_key: string) {
    ...
    try {
        ...
        if(activeUiType !== ClaimingUiType.ERROR){
