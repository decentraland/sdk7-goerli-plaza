...
;(async () => {
  ...
  await db.exec(
    'CREATE TABLE IF NOT EXISTS users (id TEXT PRIMARY KEY, wearable INTEGER, timeStamp INTEGER, visits INTEGER)'
  )
})()
...
app.post('/add-user', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (existingUser) {
      ...
      await db.run('INSERT INTO users (id, wearable, timeStamp, visits) VALUES (?, ?, ?, ?)', [
        newUser.id,
        0,
        timestamp,
        1
