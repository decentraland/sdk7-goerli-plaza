...
export class RunwayAvatar extends showMgmt.ShowEntityModel {
    ...
    constructor(id: string, model: string, invisible: boolean, start: number, position: Vector3, scale: Vector3, idelAnim?: string) {
        super(model, {
            idleAnim: idelAnim,
    ...
    runSequence() {
        ...
        if (this.runwayPosition < this.poseAnims.length) {
            ...
            utils.paths.startSmoothPath(this.entity, runwayPaths[this.start][this.runwayPosition].path, runwayPaths[this.start][this.runwayPosition].duration, 60, true, () => {
                if (this.runwayPosition == this.poseAnims.length - 1) {
                    ...
                    console.log(CLASSNAME, METHOD_NAME, this.id, 'pausing for pose', this.poseIndex, this.poseAnims[this.poseIndex], this.poseDuration[this.poseIndex])

                    const self = this
                    utils.timers.setTimeout(
                        function () {
                            console.log(CLASSNAME, METHOD_NAME, self.id, 'pose over', self.poseIndex, self.poseAnims[self.poseIndex], self.poseDuration[self.poseIndex])
                            self.poseIndex++
                            self.runSequence()
                        },
