  ...
  for (let x = 0; x <= cellRowCount; x += 1) {
    for (let z = 0; z <= cellRowCount; z += 1) {
      ...
      Transform.create(boundaryEnt, {
        ...
        scale: Vector3.create(0.1, height / 2, 0.1),
...
export function initBoidController() {
  ...
  controlHelper.addObstacle('big-trunk.35', Vector3.create(9.5, 4.5, 6.5), 0.75)
  controlHelper.addObstacle('big-trunk.4', Vector3.create(8, 4, 7), 0.5)

  controlHelper.addObstacle('rock.gray.big', Vector3.create(2.2, 0, 2.2), 1.5)
  controlHelper.addObstacle('rock.gray.small', Vector3.create(8.5, -0.5, 3.5), 1)
