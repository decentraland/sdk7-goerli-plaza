...
export default class BoidsController {
   ...
   */
  constructor(
    boundaryX = 500 * BOID_CONFIG.AREA_SCALE,
    boundaryY = 500 * BOID_CONFIG.AREA_SCALE,
    boundaryZ = 500 * BOID_CONFIG.AREA_SCALE,
    subDivisionCount = 1
  ) {
  ...
  iterate(start: number = 0, end: number = this.flockEntities.length) {
    ...
    for (let i = start; i < end; i++) {
      ...
      const vx =
        ...
        this.separationWeight * sepVel[0] +
        this.obstacleWeight * obsVel[0] +
        this.seekWeight * seekVel[0] +
        this.avoidWeight * avoidVel[0]
      const vy =
        this.aligmentWeight * aligmentVel[1] +
        this.cohesionWeight * cohVel[1] +
        this.separationWeight * sepVel[1] +
        this.obstacleWeight * obsVel[1] +
        this.seekWeight * seekVel[1] +
        this.avoidWeight * avoidVel[1]
      const vz =
        this.aligmentWeight * aligmentVel[2] +
        this.cohesionWeight * cohVel[2] +
        this.separationWeight * sepVel[2] +
        this.obstacleWeight * obsVel[2] +
        this.seekWeight * seekVel[2] +
        this.avoidWeight * avoidVel[2]
  ...
  computeSeekOrAvoid(entity: BoidEntity, radius: number, dir: number, type: number) {
    ...
    this.grid.getEntitiesInCube(entity.x, entity.y, entity.z, radius, (currentEntity: BoidEntity) => {
      if (
        ...
        currentEntity != entity &&
        currentEntity.getType() === type && //|| currentEntity.getType() == EntityX.PREDATOR_ENTITY) &&
  ...
  computeObstacles(entity: BoidEntity) {
    ...
    this.grid.getEntitiesInCube(entity.x, entity.y, entity.z, obstacleRadius, (currentObstacle: BoidEntity) => {
      ...
      if (
        currentObstacle.getType() != BoidEntity.OBSTACLE_ENTITY &&
        currentObstacle.getType() != BoidEntity.PREDATOR_ENTITY
      )
