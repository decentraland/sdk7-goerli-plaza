import { CameraModeArea, CameraType, Entity, GltfContainer, InputAction, PointerEventType, RaycastQueryType, Transform, engine, inputSystem, raycastSystem } from "@dcl/sdk/ecs"
    ...
    engine.addSystem(() => {
        ...
        if (result) {
            ...
            if (result.hit && result.hit.position && result.hit.normalHit) {
                ...
                const newPos: Vector3 = Vector3.add(Vector3.add(cameraTransform.position, forwardVector), Vector3.create(0, 0.11, 0))
...
function reflectRay(hitPoint: Vector3, reflectedVector: Vector3, reflectCount: number) {
    ...
    raycastSystem.registerGlobalDirectionRaycast(
        ...
        function (result) {
            ...
            utils.tweens.startScaling(reflectedRay.reflectedRayEntity as Entity, startSize, endSize, timeToTravel, utils.InterpolationType.LINEAR, onFinish)

            // Reflected ray dissipates after a period proportional to the travelled distance
            utils.timers.setTimeout(
                function () {
                    utils.tweens.startScaling(reflectedRay.reflectedRayEntity as Entity, endSize, Vector3.create(0, 0, endSize.z), 0.5)
                },
                2000 * timeToTravel
            )
        }
