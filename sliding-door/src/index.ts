...
pointerEventsSystem.onPointerDown(leftDoor, () => utils.toggles.flip(parentDoor), { button: InputAction.IA_PRIMARY, hoverText: "Open / Close" })
pointerEventsSystem.onPointerDown(rightDoor, () => utils.toggles.flip(parentDoor), { button: InputAction.IA_PRIMARY, hoverText: "Open / Close" })
...
utils.toggles.addToggle(parentDoor, utils.ToggleState.Off, (value) => {
	if (value == utils.ToggleState.On) {
		...
		utils.tweens.startTranslation(rightDoor, rightDoorClosed, rightDoorOpen, 1, utils.InterpolationType.EASEOUTEBOUNCE)
	} else {
