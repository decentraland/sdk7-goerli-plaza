...
function createBall(
  ...
  for (const [screen] of engine.getEntitiesWith(ArcadeScreenFlag)) {
    if (
      ArcadeScreenFlag.getOrNull(screen)?.game ===
      HasGameLoaded.getOrNull(engine.RootEntity)?.game
    ) {
...
export function shoot(direction: Vector3): void {
  for (const [_paddleEntity, _paddleFlag, transform] of engine.getEntitiesWith(
    PaddleFlag,
    Transform
  )) {
    const paddlePosition = transform.position
    const spawnPosition = Vector3.create(
      paddlePosition.x,
      PLANE_HEIGHT,
      paddlePosition.z + 1
    )
    // Ball
    createBall(
      { position: spawnPosition, scale: Vector3.create(0.3, 0.1, 0.4) },
      direction
    )
  }
}
