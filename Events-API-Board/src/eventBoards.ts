import { Entity, GltfContainer, InputAction, Material, MaterialTransparencyMode, MeshCollider, MeshRenderer, Schemas, TextShape, Transform, TransformType, engine, pointerEventsSystem } from "@dcl/sdk/ecs";
...
export async function createEventBoard(
  currentEvent: number,
  interval: number,
  transform: Partial<TransformType>) {
    ...
    // Title
    const title = engine.addEntity(true)
    ...
    // coordinates
    const coordinates = engine.addEntity(true)
